...
public class Application implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		try {
			...
			System.out.println("Lookup the boss of each person...");
			for(Person person : new Person[] { greg, roy, craig }) {
				System.out.println("The " + person.name + " boss is " + (person.boss != null ? person.boss.name : "nobody"));
			}
			
			System.out.println("Lookup the employee of each boss...");
			for(String name : new String[] { greg.name, roy.name, craig.name }) {
				System.out.println("The " + name + " employees are " + 
									(personRepository.findByBossName(name) != null ? personRepository.findByBossName(name).name : "nobody"));
			}
			
			Scanner keyboard = new Scanner(System.in);
			System.out.println("Já mostrou o neoclipse?");
			keyboard.nextLine();
			keyboard.close();
